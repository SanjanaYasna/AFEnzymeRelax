...
class NodeEmbeddingBlock(torch.nn.Module):
    def __init__(self, one_hot_dim, hidden_channels):
        super().__init__()
        self.emb = Linear(one_hot_dim, hidden_channels*2)
        self.lin_angle_geom = Linear(3, hidden_channels)
        self.lin_angle_final = Linear(hidden_channels*3, hidden_channels*2)
    ...
    def forward(self, x, angle_geom, pos):
        ...
        angle_geom = self.lin_angle_geom(angle_geom)
        ...
        #flatten last two dimensions of rbf I DON"T TRUST THIS
        ...
        """
        embs = torch.cat((x, angle_geom), -1)
        return embs
    
class InitialInteraction(torch.nn.Module):
    def __init__(self, hidden_channels, one_hot_dim = 23,
                num_spherical=7, 
                num_radial=6, 
                cutoff = 5.0,
                envelope_exponent=5,
    ...
    def forward(self, x: torch.Tensor, angle_geom: torch.Tensor, ca_coords: torch.Tensor, edge_index: torch.Tensor) -> torch.Tensor:
        ...
        x_orig_cloned = x_orig.clone().detach()
        ...
        for _ in range(self.interaction_layers):
            interaction_weights = self.interaction_block(
                x_orig_cloned, rbf, sbf, idx_kj,  i) #DO YOU NEED X_ORIG_CLONED, or does x_orig still work without problems?
