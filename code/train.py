...
for epoch in range(0, num_epochs+1):
    ...
    for i, data in enumerate(train_loader):
        ...
        with torch.no_grad(): #computations on cpu (seems ok so far)
            ...
            ego_label_confusion_matrix = sklearn_confusion_matrix(ego_labels.detach().cpu().numpy(), func_probability.detach().cpu().numpy() > 0.5)
        #updates
        loss.backward()
        optimizer.step()
    ...
    torch.save(model.state_dict(), f"/kuhpc/work/slusky/syasna_sta/func_pred/AFEnzymeRelax/rpn_runs/GRPN_region_pred_only/epoch_{epoch}.pt")
