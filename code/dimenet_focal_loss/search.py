...
class ProteinDataLoader(DataLoader):
    def __init__(self, pt_path):
        self.files = list(os.listdir(pt_path))[:100]
...
"""dir params"""
loss_dir = "/kuhpc/work/slusky/syasna_sta/func_pred/code/dimenet_focal_loss/losses_attempts.csv"
...
for alpha in alphas:  
    for red in reduction:  
        loss_file.write(f"alpha: {alpha}, reduction: {red}\n")
        for epoch in range(0, num_epochs):
            ...
            for i, data in enumerate(train_loader):
                ...
                #DATA.Y LOSS
                data_y_loss = focal_loss_node_list(node_scores, node_list, target_res, reduction=red,
                                                alpha = 0.1)

                #DATA.EGO_LABELS LOSS
                func_probability = torch.squeeze(func_probability)
                ego_label_loss = focal_loss(func_probability, ego_labels, reduction = red)
