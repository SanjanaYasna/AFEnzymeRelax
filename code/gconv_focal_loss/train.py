...
class ProteinDataLoader(DataLoader):
    def __init__(self, pt_path):
        self.files = list(os.listdir(pt_path))[25:35]
...
"""dir params"""
loss_dir = "/kuhpc/work/slusky/syasna_sta/func_pred/code/gconv_focal_loss/losses.csv"
...
for epoch in range(0, num_epochs+1):
    ...
    for i, data in enumerate(train_loader):
        ...
        #DATA.Y LOSS
        data_y_loss = focal_loss_node_list(node_scores, node_list, target_res, reduction="mean",
                                        alpha = 0.25)

        #DATA.EGO_LABELS LOSS
        func_probability = torch.squeeze(func_probability)
        ego_label_loss = focal_loss(func_probability, ego_labels, reduction = "mean", alpha = 0.25)
